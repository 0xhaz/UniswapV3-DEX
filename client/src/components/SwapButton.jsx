...
const SwapButton = ({
  ...
  const isInsufficientBalance = () =>
    inputAmount > Utils.hexToHumanAmount(inputWalletBalance, inputDecimals);
  const isNotSigner = () => !signer;
  const isMissingApproval = () =>
    !isApproved &&
    inputSymbol !== "ETH" &&
  ...
  useEffect(() => {
    ...
    if (inputSymbol === "ETH" || inputSymbol === "" || inputSymbol === null)
      return;
  ...
  const updateButton = () => {
    ...
    if (isDisabled()) {
      ...
      if (isAskingPermission) {
        return setText("Approve in your wallet");
      }
      if (isTransacting) {
        return setText("Swapping");
      }
      if (isConfirming) {
        return setText("Confirming");
      }
      if (isApproving) {
        return setText("Approving");
      }
      if (isMissingSymbol()) {
        return setText("Select a token");
      }
      if (isMissingInputAmount()) {
        return setText("Enter amount");
      }
      if (isInsufficientBalance) {
        return setText("Insufficient balance");
      }
      return;
    }
